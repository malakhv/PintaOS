...
function build() {
    env_setup
    as start.asm -o $OUT_PATH/sboot.o
    ld -Ttext 0x7c00 --oformat=binary $OUT_PATH/sboot.o -o $OUT_PATH/sboot.bin
    echo "${COLOR_GREEN}Build S-Boot - done!${COLOR_RESET}"
}

# Make image file with S-Boot in MBR
function make_image() {
    env_setup
    dd if=/dev/zero of=$OUT_PATH/image.img bs=512 count=2880
    dd if=$OUT_PATH/sboot.bin of=$OUT_PATH/image.img
